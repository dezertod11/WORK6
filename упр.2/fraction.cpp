...
namespace Rat
{
    Rational::Rational(int num, unsigned int den) :numerate(num), denominator(den)
    {
        ...
        {
            denominator = 1;
            std::cout << "Ошибка: в знаменателе 0. Мы преобразовали число в " << numerate;
        }
        reduct();
    }
    ...
    std::istream& operator>> (std::istream& in, Rational& number)
    {
        ...
        else
        {
            ...
            std::cout << "Ошибка: в знаменателе 0. Мы преобразовали число в " << number.numerate;
        }
        number.reduct();
        return in;
    }
    ...
    Rational operator/(const Rational& first, const Rational& second)
    {
        ...
        if (!result.denominator)
        {
            ...
            std::cout << "Ошибка: в знаменателе 0. Мы преобразовали число в " << result.numerate;
        }
        result.reduct();
        return result;
    }
